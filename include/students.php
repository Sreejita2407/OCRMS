...
class Student {
	...
	function single_classtudent($id=""){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tblname." 
	...
	function single_studentclass($id="",$instrutor=""){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tblname." 
				Where IDNO= '{$id}' AND ACCOUNT_USERNAME= '{$instrutor}' LIMIT 1");
			$cur = $mydb->loadSingleResult();
			return $cur;
	}
	function single_student_class($id=""){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tblname." 
				Where CLASSID= '{$id}' LIMIT 1");
			$cur = $mydb->loadSingleResult();
			return $cur;
	}
	function studAuthentication($U_USERNAME,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblstudent` WHERE  `ACC_USERNAME` = '". $U_USERNAME ."' AND `ACC_PASSWORD` = '". $h_pass ."' GROUP BY IDNO");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $student_found = $mydb->loadSingleResult();
			...
			$_SESSION['COURSE']  		= $student_found->COURSE;
	...
	public function updatestudentclass($id="",$instrutor="") {
	  global $mydb;
		$attributes = $this->sanitized_attributes();
		...
		$sql .= " WHERE IDNO= '{$id}' AND ACCOUNT_USERNAME= '{$instrutor}'";
	  $mydb->setQuery($sql);
